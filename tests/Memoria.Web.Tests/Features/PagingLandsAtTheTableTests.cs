...
/// top of the table it pages, the way the pager inside a detail page's events panel lands at the
/// panel.
///
/// The landing place is the table itself, marked so it can be given focus as well as scrolled to:
/// a jump that moves the view but not the focus leaves a reader on the keyboard tabbing on from the
/// page they have just left.
/// </summary>
public class PagingLandsAtTheTableTests
{
    /// <summary>The name the table is landed on by, which is the one its page links carry.</summary>
    private const string Anchor = "rows";
    ...
    public async Task Marks_the_top_of_the_table_as_the_place_a_page_lands(string address)
    {
        ...
        Table(page).Should().Contain($"id=\"{Anchor}\"", "a page link has to have somewhere to land").And
            .Contain("tabindex=\"-1\"", "landing on it means being given focus, not only scrolled to");
    }

    [Theory]
    [MemberData(nameof(DataPages))]
    public async Task Lands_every_page_link_at_the_top_of_the_table(string address)
    {
        using var web = MemoriaWeb.Open().WithSampleTypes();
        await Fill(web);
    ...
    [Theory]
    [MemberData(nameof(DataPages))]
    public async Task Lands_a_change_of_size_at_the_top_of_the_table(string address)
    {
        using var web = MemoriaWeb.Open().WithSampleTypes();
        await Fill(web);
    ...
    /// <summary>The table alone, so an id anywhere else on the page cannot stand in for it.</summary>
