/*
    Memoria 1.5.0 — event store schema install (SQL Server)

    Creates the four tables the Entity Framework Core store needs, with their keys, indexes and
...
/* --------------------------------------------- aggregate-to-event link rows */
IF OBJECT_ID(N'[dbo].[DomainAggregateEvents]', N'U') IS NULL
BEGIN
    CREATE TABLE [dbo].[DomainAggregateEvents] (
        [AggregateId] nvarchar(255) NOT NULL,
        [EventId] nvarchar(450) NOT NULL,
        [AppliedDate] datetimeoffset NOT NULL,
        CONSTRAINT [PK_DomainAggregateEvents] PRIMARY KEY ([AggregateId], [EventId]),
        CONSTRAINT [FK_DomainAggregateEvents_DomainAggregates_AggregateId] FOREIGN KEY ([AggregateId])
            REFERENCES [dbo].[DomainAggregates] ([Id]) ON DELETE CASCADE,
        CONSTRAINT [FK_DomainAggregateEvents_events_EventId] FOREIGN KEY ([EventId])
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes
               WHERE name = N'IX_DomainAggregateEvents_EventId'
                 AND object_id = OBJECT_ID(N'[dbo].[DomainAggregateEvents]'))
BEGIN
    CREATE INDEX [IX_DomainAggregateEvents_EventId]
        ON [dbo].[DomainAggregateEvents] ([EventId]);
END;
